...
public class NewNewGameController {
    ...
    private MapLoaderUIHandler mapLoaderUIHandler;
    ...
    public void initialize() {
        ...
        Platform.runLater(() -> {
            this.primaryStage = (Stage) storyModeButton.getScene().getWindow();
            this.mapLoaderUIHandler = new MapLoaderUIHandler(
                    new MapLoaderService(new MapDAO(), new MonsterDAO()),
                    primaryStage
            );
        });
    }
    ...
    private void showStoryPopup() {
        if (storyPopup == null) {
            ...
            for (Map map : allLevelMaps) {
                Button mapButton = new Button("Map Level " + map.getLevel());
                mapButton.setPrefWidth(150);
                mapButton.setOnAction(e -> {
                    storyPopup.hide();
                    mapLoaderUIHandler.showStartMap(map.getId());
                });
